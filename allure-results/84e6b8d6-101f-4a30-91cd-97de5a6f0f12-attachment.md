...
- Name: signup.spec.js >> unsuccess signup
- Location: tests\signup.spec.js:16:5
...
```ts
  ...
  3  | import testdata from '../utils/demoblazedata.json' assert {type: 'json'}
  4  | import { only } from 'node:test';
  5  | test.beforeEach(async({page})=>{
  6  |     await page.goto("https://www.demoblaze.com")
  7  | })
  8  | test('signup new user',async({page})=>{
  9  |     const usernamevalue = testdata[0].signupusername
  10 |     const passwordvalue = testdata[0].signuppassword
  11 |     const homepage = new HomePage(page)
  12 |     await homepage.signupnewuser()
  13 |     await homepage.signupcredentials(usernamevalue,passwordvalue)
  14 |     await homepage.successsignup()
  15 | })
  16 | test('unsuccess signup',async({page})=>{
  17 |     const usernamevalue = testdata[0].signupusername
  18 |     const passwordvalue = testdata[0].signuppassword
> 19 |     const homepage = new HomePage(page)
     |                      ^ ReferenceError: HomePage is not defined
  20 |     await homepage.signupnewuser()
  21 |     await homepage.signupcredentials(usernamevalue,passwordvalue)
  22 |     await homepage.closebuttonclick()
  23 | })
```